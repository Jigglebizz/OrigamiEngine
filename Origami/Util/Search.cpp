...
//---------------------------------------------------------------------------------
size_t BinarySearch64( uint64_t value, const void* buf, size_t stride, size_t len )
{
  const uint8_t* u8_buf = (const uint8_t*)buf;
  ...
  size_t search_idx     = len >> 1;
  uint64_t idx_value    = *(uint64_t*)&u8_buf[ search_idx * stride ];
  ...
  while ( idx_value != value )
  {
    ...
    search_idx = ( ( end - beginning ) >> 1 ) + beginning;
    idx_value  = *(uint64_t*)&u8_buf[ search_idx * stride];
  }
